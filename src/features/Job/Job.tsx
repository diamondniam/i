...
import { useApiGet, useOpimizedAnimations } from "@/hooks";
...
import { JobOTTType } from "@/types/OTT";
import JobSkeleton from "@/features/Job/JobSkeleton";
...
export default function Job() {
  ...
  const jobs = useApiGet<"jobs", JobOTTType[]>({
    table: "jobs",
    options: {
      cache: {
        key: "jobs",
          ...
          <motion.div key="timeline">
            <Timeline>
              {jobs.data
                .sort(
                  (a, b) =>
                    new Date(b.header.dateRange[0]).getTime() -
                    new Date(a.header.dateRange[0]).getTime()
                )
                .map((job, index) => (
                  <TimelineItem key={job.id} {...job} index={index}>
                    ...
                    {job.codePreview && <Code code={job.codePreview} />}
                  </TimelineItem>
                ))}
            </Timeline>
          </motion.div>
